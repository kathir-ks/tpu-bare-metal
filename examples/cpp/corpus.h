// corpus.h — embedded public-domain training text + a char vocabulary, shared
// by the single-device and data-parallel GPT examples.
#pragma once
#include <algorithm>
#include <map>
#include <string>
...
static const char* CORPUS = R"TXT(Alice was beginning to get very tired of sitting by her sister on the
...
the world she was to get out again. The rabbit-hole went straight on like
a tunnel for some way, and then dipped suddenly down, so suddenly that
Alice had not a moment to think about stopping herself before she found
herself falling down a very deep well. Either the well was very deep, or
...
struct Vocab {
    ...
    void build(const std::string& text) {
        ...
        for (char c : chars) { stoi[c] = (int)itos.size(); itos.push_back(c); }
    }
    int size() const { return (int)itos.size(); }
    std::vector<int32_t> encode(const std::string& t) const {
        std::vector<int32_t> v; v.reserve(t.size());
        for (char c : t) { auto it = stoi.find(c); if (it != stoi.end()) v.push_back(it->second); }
        return v;
    }
};
