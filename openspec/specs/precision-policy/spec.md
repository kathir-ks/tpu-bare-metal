...
Support a selectable per-graph matmul precision policy in the graph builder
(DEFAULT bf16-equivalent for throughput, HIGHEST f32 for accuracy), so
benchmarks run fast while the finite-difference gradcheck remains accurate.
