...
- [x] 5.3 Harden numerical stability (softmax/cross_entropy/rmsnorm/logsumexp in large-magnitude and near-zero regimes) — `layers_stability.py` + `layer_table_stability.hpp`: softmax @ logits~1e3 (exact, no overflow), rmsnorm @ ~1e-4 (eps holds, exact) and ~1e3, cross_entropy @ logits~±60 (logsumexp stable); 10/10 green on TPU. Added `scale`/`fwd_tol` fields to gen_layers. No instability found — stabilization already correct.
...
- [x] 8.1 Implement each prioritized new op as a closed unit: enum + builder + `emit_node()` + `grad()` VJP + gradcheck + oracle parity together — added Slice + Pad (mutually-dual VJPs) AND Concat (VJP = slice each operand region from the gradient). All forward+grad exact on TPU, robustness covered, regression clean. (Compare/Select/Convert/Iota added earlier in misc.) Remaining candidate: Pow.
